    ...
    products.forEach((product, index) => {
        ...
        decreaseBtn.addEventListener('click', () => {
            if (product.quantity > 1) {
                ...
                priceElement.textContent = `${product.product.price *30* product.quantity} EGP`;
                // Update the localStorage with the updated cart
                localStorage.setItem("skembUserCart", JSON.stringify(products));
                updateSummary(products, subtotalElement, discountElement, totalElement , products);
...
const displayCartSummary = (usercart) => {
    ...
    let catSummaryContainer = `
        ...
        <hr>
        <p class="d-flex my-1 w-100 justify-between align-center"><span>Total Price</span><span class="subtotal">${subTotalPrice + 30} EGP</span></p>
