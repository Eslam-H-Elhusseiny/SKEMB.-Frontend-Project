    ...
    card.innerHTML = `
        ...
        <div class="card px-2 py-2 h-100">
            <div class="card-product position-relative justify-end d-flex flex-col align-center">
                ...
                <button --data-pId=${product.id} class="btn addToCart w-75 cursor-pointer d-inline-block">Add To Cart</button>
                <button class="btn addToCartLoader w-75 cursor-pointer d-none"><i class="fa-solid fa-spinner fa-spin"></i></button>
            ...
            <div class="card-text px-1 d-flex flex-col">
                <a href="${product.category}" class="product-cat mt-4">${ product.category }</a>
                <a href="cart.html?id=${ product.id }"><h3 class="product-h mb-1">${product.title.slice( 0, 25 )}</h3></a>
